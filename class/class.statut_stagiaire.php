...
class statut_stagiaire extends POG_Base
{
     ...
     * @param int limit
     * @return array $statut_stagiaireList
     */
    function GetList($fcv_array = array(), $sortBy='', $ascending=true, $limit='')
    {
        $connection = Database::Connect();
        $sqlLimit = ($limit != '' ? "LIMIT $limit" : '');
        $this->pog_query = "select * from `statut_stagiaire` ";
        $statut_stagiaireList = Array();
    ...
    function Save()
    {
        ...
        {
            $this->pog_query = "insert into `statut_stagiaire` (`libelle_stat`) values (
    ...
    function DeleteList($fcv_array)
    {
        ...
        {
            $connection = Database::Connect();
            $pog_query = "delete from `statut_stagiaire` where ";
            for ($i=0, $c=sizeof($fcv_array); $i<$c; $i++)
            {
                if (sizeof($fcv_array[$i]) == 1)
                {
